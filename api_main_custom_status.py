...
# Where to put per-project CSVs. Keep this separate from api_main_v1.5.py output.
output_dir = os.getenv("CUSTOM_STATUS_OUTPUT_DIR", "project_csvs_custom_status")
custom_status_field_id_filter = (os.getenv("CUSTOM_STATUS_FIELD_ID") or "").strip().lower()
...
def _first_present(row: dict, names: list[str]) -> str:
    normalized = {
        _clean_text(key).lower().replace(" ", "").replace("_", ""): value
...
def _is_custom_status_changelog_item(item: dict) -> bool:
    """
    Match custom fields named Status and exclude Jira's system status field.

    Jira's system field usually appears as field='status', fieldtype='jira',
    fieldId='status'. The rows this script targets appear as field='Status',
    fieldtype='custom', and/or fieldId='customfield_XXXXX'.
    """
    field_name = _clean_text(item.get("field")).casefold()
    field_type = _clean_text(item.get("fieldtype")).casefold()
    field_id = _clean_text(item.get("fieldId")).casefold()

    if field_name != "status":
        return False
    if custom_status_field_id_filter and field_id != custom_status_field_id_filter:
        return False
    return field_type == "custom" or field_id.startswith("customfield_")


def _custom_status_metadata(item: dict) -> dict[str, str]:
    return {
        "sourceField": _clean_text(item.get("field")),
        "sourceFieldtype": _clean_text(item.get("fieldtype")),
        "sourceFieldId": _clean_text(item.get("fieldId")),
    }
...
def dc_fetch_issue_meta_and_status_changes(issue_key: str, project_id_to_key_cache: dict[str, str]):
    """
    Fetch DC issue metadata, all available custom Status changes, and every key
    seen in the changelog so Cloud lookup can handle moved issues.
    """
    resp = None
    last_status = None
    last_text = None
    ...
    for history in histories:
        ...
        for item in history.get("items", []):
            ...
            if _is_custom_status_changelog_item(item):
...
def load_issue_id_map(path: Optional[str]) -> dict[str, str]:
    """
    Load an optional Data Center issue ID -> Cloud issue ID mapping CSV.

    Accepted source columns include dc_issue_id, server_issue_id, serverId,
    source_id, and sourceId. Accepted Cloud columns include cloud_issue_id,
    cloudIssueId, target_id, and targetId.
    """
    if not path:
        return {}
    ...
    with open(path, newline="", encoding="utf-8-sig") as f:
        ...
        if not reader.fieldnames:
            raise RuntimeError(f"Issue ID map {path} has no header row.")

        for row in reader:
            dc_issue_id = _first_present(
                row,
                [
                    "dc_issue_id",
                    "dcIssueId",
            ...
            if dc_issue_id and cloud_issue_id:
                mapping[dc_issue_id] = cloud_issue_id

    print(f"Loaded {len(mapping)} issue ID mapping(s) from {path}")
    return mapping
...
def cloud_fetch_status_changes(issue_id_or_key: str) -> list[dict]:
    """
    Return every custom Status change event recorded in Jira Cloud for an issue.
...
def cloud_resolve_issue_and_changelog(
    candidate_keys: list[str],
    mapped_cloud_issue_id: str = "",
) -> tuple[dict, Optional[list[dict]], str, str, list[str], str]:
    """
    Try a mapped Cloud issue ID first, then each known DC key as a Cloud
    ...
    mapped_cloud_issue_id = _clean_text(mapped_cloud_issue_id)
    if mapped_cloud_issue_id:
        label = f"mapped-id:{mapped_cloud_issue_id}"
        tried.append(label)
        try:
            identity = cloud_get_issue_identity(mapped_cloud_issue_id)
            cloud_issue_id = identity.get("id") or mapped_cloud_issue_id
            seen_issue_ids.add(cloud_issue_id)
            changes = cloud_fetch_status_changes(cloud_issue_id)
            return identity, changes, mapped_cloud_issue_id, "mapped_id", tried, ""
...
def build_missing_row(
    *,
    ...
    return {
        ...
        "source_field": entry.get("sourceField", ""),
        "source_fieldtype": entry.get("sourceFieldtype", ""),
        "source_fieldId": entry.get("sourceFieldId", ""),
...
def process_project(
    ...
    try:
        ...
        for key in tqdm(issue_keys, desc=f"DC changelogs {project_key}"):
            try:
                ...
                for change in status_changes:
                    if _clean_text(change["toStatus"]["name"]):
        ...
        if not dc_transitions_by_issue:
            print(
                f"[{project_key}] No DC custom Status transitions found. "
                f"Writing empty CSV with headers."
            )
            write_csv(project_key, [])
            return
        ...
        for key in tqdm(
            ...
            mapped_cloud_issue_id = issue_id_map.get(meta.get("dc_issue_id", ""), "")
            identity, changes, used, method, tried, error = cloud_resolve_issue_and_changelog(
                candidates,
                mapped_cloud_issue_id=mapped_cloud_issue_id,
            )
                ...
                if _consume_cloud_transition_match(entry, cloud_buckets) is None:
                    missing_rows.append(
                        build_missing_row(
                            ...
                            cloud_lookup_method=cloud_lookup_method.get(key, ""),
                            cloud_lookup_error="",
                            comparison_status="missing_custom_status_transition",
                        )
                    )
...
def main():
    global custom_status_field_id_filter

    parser = argparse.ArgumentParser(
        description=(
            "Compare Jira DC vs Cloud custom Status transitions "
            "(fieldtype=custom), per-project."
        )
    )
    parser.add_argument(
        "--only",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional CSV mapping Data Center issue IDs to Cloud issue IDs. "
            "When supplied, Cloud lookup uses the mapped issue ID before trying keys."
        ),
    )
    parser.add_argument(
        "--custom-status-field-id",
        dest="custom_status_field_id",
        default=custom_status_field_id_filter,
        help=(
            "Optional custom field ID to restrict matching, e.g. customfield_12345. "
            "Defaults to CUSTOM_STATUS_FIELD_ID from .env when set."
        ),
    )
    args = parser.parse_args()

    custom_status_field_id_filter = (args.custom_status_field_id or "").strip().lower()
    issue_id_map = load_issue_id_map(args.issue_id_map)

    if custom_status_field_id_filter:
        print(f"Restricting custom Status matching to {custom_status_field_id_filter}")
    else:
        print("Matching any changelog item with field='Status' and fieldtype='custom'.")
