...
def dc_get_project_key_by_id(project_id: str, _cache: dict[str, str], _lock: threading.Lock) -> str:
    """Resolve a DC project ID to its project KEY (cached). Thread-safe."""
    if not project_id:
        return ""
    with _lock:
...
def dc_fetch_issue_meta_and_status_changes(issue_key: str, project_id_to_key_cache: dict[str, str], project_id_to_key_lock: threading.Lock):
    """
    Fetch a DC issue with expand=changelog and return:
      - meta: info about potential key/project moves
      - status_changes: iterable of status-change dicts (same shape as before, plus move fields)

    Notes:
    - Jira can record a move as:
        field == 'key'     (issue key changed)
        field == 'project' (project changed; from/to are project IDs)
    - We record BOTH IDs and resolved keys for project changes.
    """
    resp = None
    last_status = None
    last_text = None
    ...
    current_key = data.get("key") or issue_key

    fields = data.get("fields", {}) or {}
    current_project = fields.get("project", {}) or {}
    current_project_id = str(current_project.get("id") or "")
    current_project_key = current_project.get("key") or ""
    if not current_project_key and current_project_id:
        current_project_key = dc_get_project_key_by_id(current_project_id, project_id_to_key_cache)
    ...
    # Track move information (if present)
    old_issue_key = current_key
    new_issue_key = current_key

    old_project_id = current_project_id
    new_project_id = current_project_id
    old_project_key = current_project_key
    new_project_key = current_project_key
    ...
    for history in histories:
        ...
        history_id = str(history.get("id") or "")  # changegroup.id

        for item in history.get("items", []):
            field = item.get("field")

            # Detect issue-key change (move between projects, or explicit key change)
            if field == "key":
            ...
            # Detect project change; from/to are typically project IDs.
            if field == "project":
                f_id = str(item.get("from") or "")
                t_id = str(item.get("to") or "")
                if f_id:
                    old_project_id = f_id
                    old_project_key = dc_get_project_key_by_id(f_id, project_id_to_key_cache)
                if t_id:
                    new_project_id = t_id
                    new_project_key = dc_get_project_key_by_id(t_id, project_id_to_key_cache)

            # Collect status changes
            if field == "status":
                status_changes.append(
                    {
                        "issueKey": issue_key,  # the key used to fetch from DC
    ...
    meta = {
        ...
        "dc_issue_key_old": old_issue_key,
        "dc_issue_key_new": new_issue_key,
...
def process_project(project_key: str, project_id: str, updated_since_days: int | None, max_workers: int):
    ...
    try:
        ...
        # Track issue moves (key/project changes) discovered in the DC changelog
        dc_issue_move_meta: dict[str, dict] = {}
        # Cache for resolving DC project IDs -> project keys (used for move reporting)
        project_id_to_key_cache: dict[str, str] = {}

        
        project_id_to_key_lock = threading.Lock()

        def _dc_worker(issue_key: str):
            meta, status_changes = dc_fetch_issue_meta_and_status_changes(
                issue_key, project_id_to_key_cache, project_id_to_key_lock
            )
            filtered = []
            for change in status_changes:
                if change["fromStatus"]["name"] and change["toStatus"]["name"]:
                    filtered.append(change)
            return issue_key, meta, filtered

        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            futures = {ex.submit(_dc_worker, k): k for k in issue_keys}

            for fut in tqdm(
                as_completed(futures),
                total=len(futures),
                desc=f"DC changelogs {project_key}",
            ):
                key = futures[fut]
                try:
                    issue_key, meta, filtered_changes = fut.result()
                    dc_issue_move_meta[issue_key] = meta

                    # Store DC issue id even if no status transitions (still useful for reporting)
                    if issue_key not in dc_issue_id_map:
                        dc_issue_id_map[issue_key] = meta.get("dc_issue_id", "")

                    for change in filtered_changes:
                        dc_transitions_by_issue[issue_key].append(change)
        ...
        cloud_issue_key_used: dict[str, str] = {}

        
        def _cloud_worker(dc_key: str):
            meta = dc_issue_move_meta.get(dc_key, {}) or {}
            preferred_cloud_key = (
                meta.get("dc_issue_key_new")
                or meta.get("dc_issue_key_current")
                or dc_key
            )
            fallback_cloud_key = meta.get("dc_issue_key_old") or dc_key
            ...
            for candidate in [preferred_cloud_key, fallback_cloud_key, dc_key]:
                if not candidate or candidate in tried:
                    continue
                tried.append(candidate)
                try:
                    pairs = cloud_fetch_status_pairs(candidate)
                    cloud_issue_id = cloud_get_issue_id(candidate)
                    break
                except Exception as e:
                    last_err = e
                    continue
            ...
            if pairs is None:
                return dc_key, None, "", "", tried, last_err
            return dc_key, pairs, cloud_issue_id, (tried[-1] if tried else preferred_cloud_key), tried, None

        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            futures = {
                ex.submit(_cloud_worker, k): k for k in dc_transitions_by_issue.keys()
            }

            for fut in tqdm(
                as_completed(futures),
                total=len(futures),
                desc=f"Cloud changelogs {project_key}",
            ):
                dc_key = futures[fut]
                try:
                    key, pairs, cloud_issue_id, used_key, tried, err = fut.result()
                except Exception as e:
                    print(
                        f"[{project_key}] Cloud worker crashed for {dc_key}: {e}. "
                        f"Skipping this issue in comparison."
                    )
                    cloud_cache[dc_key] = None
                    cloud_issue_id_map[dc_key] = ""
                    cloud_issue_key_used[dc_key] = ""
                    continue

                if pairs is None:
                    print(
                        f"[{project_key}] Cloud error for {dc_key} "
                        f"(tried: {', '.join(tried)}): {err}. "
                        f"Skipping this issue in comparison."
                    )
                    cloud_cache[dc_key] = None
                    cloud_issue_id_map[dc_key] = ""
                    ...
                    cloud_cache[dc_key] = pairs
                    cloud_issue_id_map[dc_key] = cloud_issue_id
                    cloud_issue_key_used[dc_key] = used_key
        ...
        for key, entries in tqdm(
            ...
            for entry in entries:
                ...
                row = {
                    # Original key used when enumerating issues from DC search
                    "issueKey": key,
                    "dc_issue_id": dc_issue_id_map.get(key, ""),
                    "cloud_issue_id": cloud_issue_id_map.get(key, ""),
                    # If the issue moved in Cloud, this is the key we ended up querying.
                    "cloud_issue_key_used": cloud_issue_key_used.get(key, ""),
                    # Move tracking (if any)
                    "dc_issue_key_old": (dc_issue_move_meta.get(key, {}) or {}).get("dc_issue_key_old", ""),
                    "dc_issue_key_new": (dc_issue_move_meta.get(key, {}) or {}).get("dc_issue_key_new", ""),
                    "dc_project_key_old": (dc_issue_move_meta.get(key, {}) or {}).get("dc_project_key_old", ""),
                    "dc_project_key_new": (dc_issue_move_meta.get(key, {}) or {}).get("dc_project_key_new", ""),
...
def main():
    ...
    parser.add_argument(
        "--threads",
        type=int,
        default=int(os.getenv("THREADS", "8")),
        help="Number of worker threads to use for per-issue DC/Cloud API calls (default: env THREADS or 8).",
    )
