...
def dc_get_project_key_by_id(project_id: str, _cache: dict[str, str]) -> str:
    """Resolve a DC project ID to its project KEY (cached)."""
    if not project_id:
        return ""
...
def process_project(project_key: str, project_id: str, updated_since_days: int | None):
    ...
    try:
        ...
        for key in tqdm(
            ...
            # Prefer the latest (post-move) DC key when talking to Cloud.
            preferred_cloud_key = meta.get("dc_issue_key_new") or meta.get("dc_issue_key_current") or key
            fallback_cloud_key = meta.get("dc_issue_key_old") or key

            tried = []
            pairs = None
            cloud_issue_id = ""

            for candidate in [preferred_cloud_key, fallback_cloud_key, key]:
            ...
            if pairs is None:
                ...
                print(
                    f"[{project_key}] Cloud error for {key} "
                    f"(tried: {', '.join(tried)}): {last_err}. "
                    f"Skipping this issue in comparison."
                )
                cloud_cache[key] = None
                cloud_issue_id_map[key] = ""
                cloud_issue_key_used[key] = ""
            else:
                cloud_cache[key] = pairs
                cloud_issue_id_map[key] = cloud_issue_id
                cloud_issue_key_used[key] = tried[-1] if tried else preferred_cloud_key
